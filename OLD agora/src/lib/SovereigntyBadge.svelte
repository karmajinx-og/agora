...
{#if sovereignty}
  <div class="badge level-{sovereignty.level}" class:compact>
    {#if sovereignty.level === 'safe'}
      <span class="dot"></span>{compact ? 'Safe' : 'Sovereignty safe'}
    {:else if sovereignty.level === 'caution'}
      <span class="dot"></span>{compact ? 'Caution' : 'Review advised'}
    {:else if sovereignty.level === 'risk'}
      <span class="dot"></span>{compact ? 'Risk' : 'Sovereignty risk'}
    {:else}
      <span class="dot"></span>{compact ? '?' : 'Not reviewed'}
    {/if}
  </div>
  ...
  {#if !compact && sovereignty.tags}
    <div class="tags">
      {#if sovereignty.tags.eu_origin !== null}
        <span class="tag" class:on={sovereignty.tags.eu_origin} class:off={!sovereignty.tags.eu_origin}>
          EU origin
        </span>
      {/if}
      {#if sovereignty.tags.gdpr_safe !== null}
        <span class="tag" class:on={sovereignty.tags.gdpr_safe} class:off={!sovereignty.tags.gdpr_safe}>
          GDPR safe
        </span>
      {/if}
      {#if sovereignty.tags.no_telemetry !== null}
        <span class="tag" class:on={sovereignty.tags.no_telemetry} class:off={!sovereignty.tags.no_telemetry}>
          No telemetry
        </span>
      {/if}
      {#if sovereignty.tags.open_source !== null}
        <span class="tag" class:on={sovereignty.tags.open_source} class:off={!sovereignty.tags.open_source}>
          Open source
        </span>
      {/if}
  ...
  {/if}
{:else if !compact}
  <div class="badge level-unknown">
    <span class="dot"></span>Not reviewed
  </div>
{/if}
